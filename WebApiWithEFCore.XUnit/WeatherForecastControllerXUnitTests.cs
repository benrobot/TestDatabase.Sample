...
namespace WebApiWithEFCore.XUnit
{
    ...
    public class WeatherForecastControllerXUnitTests : IDisposable
    {
        private readonly WeatherForecastController _controllerWithSqlServer;
        private readonly BloggingContext _dbContextWithSqlServerDocker;
        private readonly TestLoggerFactory _loggerFactory;

        public WeatherForecastControllerXUnitTests(DatabaseFixture databaseFixture, ITestOutputHelper testOutputHelper)
        {
            var logger = new TestLoggerToXUnitTestOutputHelper(testOutputHelper);
            _loggerFactory = new TestLoggerFactory(logger);
            var controllerLogger = _loggerFactory.CreateLogger<WeatherForecastController>();
            var dbContextOptions = new DbContextOptionsBuilder<BloggingContext>()
                .UseSqlServer(databaseFixture.DbConnectionString)
                .Options;
            _dbContextWithSqlServerDocker = new BloggingContext(dbContextOptions);
            _dbContextWithSqlServerDocker.Database.EnsureCreated();
            PrePopulateDatabase(_dbContextWithSqlServerDocker);
            _controllerWithSqlServer = new WeatherForecastController(controllerLogger, _dbContextWithSqlServerDocker);
        }
        ...
        public void Dispose()
        {
            _dbContextWithSqlServerDocker.Database.EnsureDeleted();
