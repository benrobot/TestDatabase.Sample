...
namespace WebApiWithEFCore.MSTest
{
    [TestClass]
    public class WeatherForecastControllerMSTestTests
    {
        private WeatherForecastController _controllerWithSqlServer;
        private static ILoggerFactory _loggerFactory;
        private static DbContextOptions<BloggingContext> _dbContextOptionsWithSqlServer;

        [ClassInitialize]
        public static void OneTimeSetup(TestContext testContext)
        {
            var logger = new TestLogger(testContext.WriteLine);
            ...
            var testDatabase = new SqlServerDockerDatabase(new SqlServerDockerDatabaseOptions(dockerSqlServerHostPort: 1457, stopDockerInstanceOnDispose: false), dockerSqlServerDatabaseLogger);
